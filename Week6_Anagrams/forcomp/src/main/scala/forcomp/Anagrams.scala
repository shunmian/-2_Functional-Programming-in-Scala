...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    {
      for {
        i <- x
        j <- y
      } yield if (i._1 == j._1) (i._1, i._2 - j._2) else i
    }.toList.filter(pair => pair._2 != 0)
  }
  /**
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def sentenceAnagrams0(ocs: Occurrences): List[Sentence] = ocs match {
      case Nil => List(Nil)
      case x :: xs =>
        {
          for {
            fst: Occurrences <- combinations(ocs)
            wordFst: Word <- dictionaryByOccurrences.getOrElse(fst, Nil)
            wordRest <- sentenceAnagrams0(subtract(ocs, fst))
            if !fst.isEmpty
          } yield wordFst :: wordRest
        }
    }
