...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
	...
	def union(that: TweetSet): TweetSet = {
	  left.union(right.union(that.incl(elem)))    //the most tricky part. (left union right union that incl elem) is inefficient
	}

  def mostRetweeted: Tweet = {

    val remaining = left.union(right)
    val morePopular = remaining.filter(x => x.retweets > elem.retweets)
